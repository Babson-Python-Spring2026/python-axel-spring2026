...
def create_transaction(path, date, transaction_type, ticker="", shares="", price="", cash="", factor="", note=""):
    ...
    new_df, state, cash_balance = append_transaction(df, new_row)
    save_transactions(new_df, path)
    return new_df


def get_cash_balance(path, as_of_date):
    df = load_transactions(path)
    cutoff = _normalize_date(as_of_date)
    df = df[df["DATE"] <= cutoff]
    _, cash = replay_transactions(df)
    return cash


def build_portfolio(path, as_of_date):
    df = load_transactions(path)
    cutoff = _normalize_date(as_of_date)
    df = df[df["DATE"] <= cutoff]
    shares, _ = replay_transactions(df)
    return shares


def list_transactions_for_ticker(path, ticker):
    df = load_transactions(path)
    ...
    out["__DATE_SORT"] = pd.to_datetime(out["DATE"], format="%Y%m%d")
    out["__TYPE_ORDER"] = out["TYPE"].map(TYPE_ORDER)
    out = out.sort_values(["__DATE_SORT", "__TYPE_ORDER"]).drop(columns=["__DATE_SORT", "__TYPE_ORDER"])
